...
export const getListStockQueryKey = (params?: { search?: string }) => [
  'stock',
  params ?? {},
...
export const getGetMonthlyExpensesQueryKey = () => [
  'dashboard',
  'monthly-expenses',
  getCurrentLocalUserId(),
]
export const getListExpensesQueryKey = (params?: { month?: string; date?: string }) => [
  'expenses',
  params ?? {},
  getCurrentLocalUserId(),
]
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params.search) {
        ...
        query = query.or(
          `model_code.ilike.%${search}%,name.ilike.%${search}%,brand.ilike.%${search}%`,
        )
      }
  ...
  return useMutation({
    mutationFn: async ({ data }: { data: StockInput }) => {
      ...
      const { data: row, error } = await supabase
        .from('stock_items')
        .insert(withOwner(stockInputToRow(payload)))
        .select('*')
        .single()
      throwIfError(error)
...
export function useListExpenses(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params.month)
        query = query.gte('date', `${params.month}-01`).lt('date', nextMonthStart(params.month))
      const { data, error } = await query
...
export function useCreateExpense() {
  return useMutation({
    mutationFn: async ({ data }: { data: ExpenseInput }) => {
      const { data: row, error } = await supabase
        .from('expenses')
        .insert(withOwner(expenseInputToRow(data)))
        .select('*')
        .single()
      throwIfError(error)
...
export function useCreateModelCode() {
  return useMutation({
    mutationFn: async ({ data }: { data: { code: string } }) => {
      ...
      const { data: row, error } = await supabase
        .from('model_codes')
        .insert({ code, is_default: false, user_id: userId })
        .select('*')
        .single()
      throwIfError(error)
...
export function useCreateExpenseCategory() {
  return useMutation({
    mutationFn: async ({ data }: { data: { name: string } }) => {
      ...
      const { data: row, error } = await supabase
        .from('expense_categories')
        .insert({ name, is_default: false, user_id: userId })
        .select('*')
        .single()
      throwIfError(error)
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DashboardSummary> => {
      ...
      return {
        ...
        todaySalesAmount: todaySoldItems.reduce(
          (sum, item) => sum + item.soldCount * item.salePrice,
          0,
        ),
        todayProfit: todaySoldItems.reduce(
          (sum, item) => sum + item.soldCount * (item.salePrice - item.purchasePrice),
          0,
        ),
        totalProfit: stockItems.reduce(
          (sum, item) => sum + item.soldCount * (item.salePrice - item.purchasePrice),
          0,
        ),
        thisMonthExpenses: expenseItems
...
export function useGetSyncStatus() {
  return useQuery({
    ...
    queryFn: async (): Promise<SyncStatus> => {
      ...
      throwIfError(expensesError)
      throwIfError(syncError)
      const stockPending = (stock ?? []).filter(
        row => !row.synced_at || String(row.synced_at) < String(row.updated_at),
      ).length
...
export async function exportExpensesCsv(month: string) {
  ...
  return toCsv(
    ...
    [
      ...rows.map(r => [r.date, r.description, r.amount.toFixed(2), r.category ?? '', r.createdAt]),
